...
# ──────────────────────────────────────────────────────────────────────
# Low-level download & parse helpers (duplicated from cidr_list_updater
# but without geo-filtering so we keep ALL data for the DB)
# ──────────────────────────────────────────────────────────────────────
...
def _download_text(url, timeout=45):
    req = request.Request(url, headers={"User-Agent": "AdminAntizapret-CIDR-DB/1.0"})
...
class CidrDbUpdaterService:
    ...
    def refresh_all_providers(
        ...
        *,
        ...
        from core.services.cidr_list_updater import PROVIDER_SOURCES
