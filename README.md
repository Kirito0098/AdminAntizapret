...
> - Если порт 80 занят, сервисы, использующие его, временно останавливаются, а затем перезапускаются.
> - Установка Let's Encrypt минимизирована, лишние библиотеки и задания в `cron` удалены.
> - Создано собственное задание для обновления сертификатов с временным изменением таблицы маршрутизации.
>
> **Примечание:** Улучшена проверка портов:
> - Определяется и отображается сервис, занимающий порт.
> - Проверяется наличие перенаправлений в таблице маршрутизации с отображением правил.
> 
...
- ✅ **Гибкое управление маршрутизацией через веб-интерфейс** (ROUTE_ALL, DISCORD_INCLUDE и другие параметры)
...
2. На главной странице вы можете:
   ...
   *Рисунок 8: Генерация и демонстрация QR-кода для конфигураций.*
   ...
   ![Управление маршрутизацией Antizapret](https://github.com/user-attachments/assets/544b890c-c6c3-4f43-abc6-d64fca2c7ae7)
   *Рисунок 9: Управление параметрами маршрутизации через веб-интерфейс.*
---
...
14. Проверить конфликт портов 80/443 
15. Изменить протокол (HTTP/HTTPS) 
