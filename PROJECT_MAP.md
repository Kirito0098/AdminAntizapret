...
- Документация по Telegram Mini App и связанным сценариям интеграции.

### `PROJECT_MAP.md`

- Техническая карта репозитория.
...
### `templates/tg_mini_app.html`

- Шаблон Telegram Mini App интерфейса.

### `templates/tg_mini_open.html`

- Вспомогательный Telegram Mini App entry/open шаблон.
...
- `settings_styles.css` — стили страницы настроек.
- `tg_mini_app.css` — стили Telegram Mini App.
- `theme.css` — общие переменные темы/базовые цветовые токены.
...
- `tg_mini_app.js` — клиентская логика Telegram Mini App.
...
### `tests/test_script_sh_all.sh`

- Smoke/регрессионные shell-проверки для скриптов в `script_sh/`.
...
Документ актуален для состояния репозитория на 2026-04-12.
