...
- **Поддержка Nginx как reverse proxy с автоматической настройкой Let's Encrypt**:
  ...
  - Приложение продолжает работать по HTTP на внутреннем порту — весь TLS обрабатывает Nginx.
- **Улучшенная обработка доменных имён и конфигов Nginx**:
  - Имя конфига Nginx генерируется автоматически на основе введённого домена (например, `nginx2_claymore-it_ru`).
...
- **Конфигурация Gunicorn (`gunicorn.conf.py`)**:
  - Добавлена поддержка переменной окружения `BIND` для управления адресом привязки.
  - По умолчанию `0.0.0.0`, в режиме Nginx — автоматически `127.0.0.1` для защиты от прямого доступа.

### Исправлено

- **Проблемы с установкой Certbot через snap**:
  - Устранены ошибки, связанные с отсутствием файлов конфигурации SSL.
  - Надёжная работа при повторных запусках установки.
- **Конфликты портов и правил iptables**:
  - Полное освобождение порта 80 на время получения сертификата.
  - Корректное восстановление всех правил и сервисов.
