...
### Fix авторазблокировки при увеличении лимита трафика

- **Fix OpenVPN**: `reconcile_all` больше не превращает устаревшие banlist-записи от `traffic_limit` в `is_permanent_blocked`; при `set_traffic_limit` / `clear_traffic_limit` reconcile снимает ошибочную permanent-блокировку и обновляет banlist, если потребление ниже нового лимита (`openvpn_access_policy.py`).

### Fix переносов строк в карточках клиентов

- **Fix UI карточек клиентов (главная)**: исправлены переносы после увеличения шрифтов — дата сертификата и «· N дн.» держатся в одной группе, алерт блокировки в две строки (причина / дата), трафик «Превышен» на отдельной строке под значениями, подпись «БЫЛ» единообразно (`styles_index.css`, `_macros.html`, `page-core.js`).

### UI карточек клиентов — читаемость типографики

- **UI карточек клиентов (главная)**: увеличены размеры шрифтов на карточках сетки — имя, бейджи статуса, онлайн-строка, подписи и значения 7д/30д, трафик, сертификат/«Был», чипы протоколов, кнопка «Открыть действия и график», текст блокировки; вторичный текст чуть ярче, карточкам добавлены отступы и min-height для 5-колоночной сетки (`styles_index.css`).

### Fix UI карточек клиентов (главная)

- **Fix**: все карточки клиентов на главной теперь показывают блок статистики (онлайн, 7д/30д, «Был») — при отсутствии данных в `client_details_payload` отображаются «—» / «нет данных», а не пустая секция; сопоставление имён клиента с payload без учёта регистра (`page-core.js`, `client_details.py`, `styles_index.css`).

### UI карточек клиентов (главная, v3)

- **UI карточек клиентов (главная, v3)**: компактный layout — бейджи статуса и сертификата в одной строке, сетка статистики 2×2 (онлайн с пульсирующей точкой, трафик 7д/30д), «Был» из `last_seen_at`, подписи «Сертификат»/«Срок» по протоколу, компактный алерт блокировки, меньше вертикальных отступов у активных карточек, усиленный hover (`_macros.html`, `styles_index.css`, `page-core.js`).

### UI модалки клиента (главная)

- **UI модалки «Открыть действия и график»**: переработан layout — липкий заголовок со статус-чипами, сводка и трафик в stat-pills, ограничения вынесены в отдельную секцию без вложенных рамок, сегментированные кнопки диапазона графика, мета-строка и линия лимита заметнее, состояния загрузки со спиннером (`_client_details_modal.html`, `styles_index.css`, `client-details.js`).

### Политики доступа клиентов

- **Fix**: бейдж «Сертификат истёк» на главной больше не показывается для свежих 1-дневных сертификатов — `days_left=0` при остатке <24 ч теперь считается «истекающим», а не просроченным (`page_context.py`, `access_remaining.py`).

- **UI карточек клиентов (главная)**: сетка на широких экранах — 5 карточек в ряд вместо 6 (`styles_index.css`, breakpoint ≥1500px).
- **UI карточек клиентов (главная)**: переработан layout строк таблицы — имя и статус в одной строке, блокировка и авторазблокировка выделены отдельно, трафик с mini-bar и крупным «использовано», срок доступа приглушён; убрано дублирование лимита в тексте блокировки.
- **Fix**: исправлен 500 на главной при клиентах с лимитом трафика — ошибка Jinja `round` на списке в расчёте процента progress-bar (`templates/partials/index/_macros.html`).
- **Fix UI карточек клиентов**: полное имя (перенос вместо «AN…»), независимая высота карточек в ряду (`align-items: start`), у активных клиентов — строка «Трафик · Лимит не задан» для сбалансированного вида.
- **UI карточек клиентов (главная, v2)**: компактная блокировка и лимит трафика, стат-пилюли онлайн/7д/30д из `client_details_payload`, бейдж срока сертификата, цветной акцент статуса, кнопка «Открыть действия» внизу карточки, сетка 5 колонок на широких экранах.

- **Блокировка по объёму трафика** для OpenVPN и WG/AWG: лимит в БД (`traffic_limit_bytes` в `wg_access_policy` / `openvpn_access_policy`), учёт потреблённого трафика из `user_traffic_stat_protocol`, автоматическая блокировка при превышении (`block_reason=traffic_limit`, `block_mode=traffic_limit`).
- **Периодические лимиты трафика (1 / 7 / 30 дней)**: поле `traffic_limit_period_days` в политиках доступа; потребление считается по **календарным** периодам (UTC): сутки 00:00–23:59:59, неделя пн–вс, месяц с 1-го по последний день. Блокировка по `traffic_limit` **временная** — при наступлении нового периода reconcile автоматически снимает её (потребление обнуляется для нового окна). Для legacy-лимитов без периода — по-прежнему all-time из `user_traffic_stat_protocol`.
- **API**: `POST /api/wg/client-access` и `POST /api/openvpn/client-block` — действия `set_traffic_limit` (`limit_value`, `limit_unit`: MB/GB/TB, `limit_period_days`: 1/7/30) и `clear_traffic_limit`; при попытке разблокировки без снятия лимита — `409` с `error_code=traffic_limit_exceeded`.
- **UI главной**: в модалке лимита — выбор периода (1 день / 7 дней / 30 дней); в карточке клиента отображаются лимит, период, использовано и остаток; на графике «Трафик клиента (БД)» — линия накопленного трафика и пунктирная линия лимита с подписью.
...
- **Telegram-уведомления по лимиту трафика**: при автоматической блокировке (`traffic_limit_block`) и авторазблокировке в начале нового периода (`traffic_limit_unblock`); событие `traffic_limit` в настройках TG-уведомлений пользователя; дедупликация по периоду в памяти процесса (без повторов на каждой сверке); сообщения на русском с протоколом, лимитом, периодом, потреблением и временем авторазблокировки.
