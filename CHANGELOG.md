...
- **Умная кнопка обновления системы** в веб-интерфейсе:
  - Автоматическая проверка наличия обновлений при загрузке страницы.
  - Два состояния кнопки:
    - **Красная** «Доступно обновление!» — если есть новые коммиты в репозитории.
    - **Зелёная** «У вас последняя версия» — если панель полностью актуальна (кнопка блокируется).
  - Добавлен эндпоинт `/check_updates` для быстрого сравнения локальной и удалённой версии через `git fetch`.
- **Принудительное обновление системы** (с подтверждением):
  - Полный `git reset --hard origin/main` + `git clean -fd`.
  - Автоматическая установка зависимостей и перезапуск сервиса `admin-antizapret.service`.
  - Все локальные изменения безвозвратно удаляются (с двойным предупреждением пользователю).
...
- **Новые параметры маршрутизации в Antizapret**:
  - `ROBLOX_INCLUDE` – перенаправление трафика Roblox через Antizapret (игровой контент, платежи, социальные функции).
  - `WHATSAPP_INCLUDE` – перенаправление трафика WhatsApp через Antizapret (обмен сообщениями, звонки, медиафайлы).

### Изменено
- Обновлён заголовок приложения: переименован с "AdminVPN Claymore" на **"AdminPanel Claymore"**.
- Улучшена обработка ошибок при обновлении: теперь даже при проблемах с `systemctl` обновление считается успешным (файлы обновляются в любом случае).
- Кнопка обновления теперь стилизована под текущий статус (красный/зелёный фон).
