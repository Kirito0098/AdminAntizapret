...
- Таблица **«База данных CIDR»**: убрана колонка AS (данные в БД и метаданных); баннер предупреждений скрывает уровни `info`/`none`.
...
- Расширены **`tests/test_cidr_db_updater_service.py`**: параллельные источники, retry ASN, fallback-пул, RIPE-only провайдеры (Akamai, DigitalOcean, Cloudflare), отсутствие bgp.tools, очистка БД и алерты после `cleared`.
