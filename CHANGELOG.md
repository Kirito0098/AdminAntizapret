...
- Настройка «Бэкап AntiZapret (client.sh 8)» на вкладке **Обслуживание системы**; увеличен timeout отправки документов в TG для крупных архивов.
- Кнопка **«Тест: бэкап и Telegram»** — ручной прогон бэкапов панели + AZ и отправка архивов выбранным админам (`POST /api/backups/test-telegram`), без включения авто-бэкапа по расписанию.
- Если полный архив панели **больше 50 МБ** (лимит Telegram), в чат отправляются **.env + data/** и текст с путём к полному бэкапу на сервере.
