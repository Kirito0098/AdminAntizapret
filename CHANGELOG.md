...
- Внутри четыре вкладки: **Провайдеры** (выбор облаков и CDN), **Игровые фильтры** (игровые серверы), **Регионы** (фильтрация по странам) и **Синхронизация** (применение изменений).

### Управление провайдерами и IP-списками

- Теперь можно выбирать, трафик каких крупных провайдеров направлять через AntiZapret — Cloudflare, Google, Amazon, Akamai, DigitalOcean, Hetzner, OVH.
- Для каждого провайдера показано, какие сервисы он обслуживает (например, Google — это YouTube, Gmail, Google Play; Amazon — Twitch, Slack, Discord).
- Добавлена кнопка **«Синхронизировать»** — применяет выбранные настройки и перезапускает обновление маршрутов без ручных действий.

### Игровые фильтры

- Возможность включить маршрутизацию через AntiZapret для конкретных игровых серверов — без необходимости добавлять их вручную.
- Фильтры можно комбинировать с провайдерскими списками.

### Умное ограничение маршрутов для iPhone и iPad

- Добавлена защита от переполнения таблицы маршрутов на устройствах Apple (iOS поддерживает не более ~900 маршрутов в OpenVPN).
- Панель автоматически следит за лимитом и не даёт подключить слишком много списков одновременно.

### Вкладка «Обновления» в настройках

- В разделе **Настройки** появилась вкладка **«Обновления»**: показывает, актуальная ли сейчас версия панели.
...
- Обновлены адреса Cloudflare, Google, DigitalOcean, Hetzner, OVH до актуальных.
- Удалены устаревшие провайдеры M247 и Zenlayer, которые больше не актуальны.
