...
### Блокировка IP-сканеров и firewall

- Новый модуль `scanner_firewall_store`: персистентное хранилище банов (`data/scanner_blocks.json` по умолчанию), учёт «страйков» и длительных блокировок, опциональная синхронизация с **ipset/iptables** (включение и сухой прогон задаются переменными окружения `IP_SCANNER_FIREWALL_ENABLED`, `IP_SCANNER_FIREWALL_DRY_RUN` и связанными `IP_SCANNER_*`).
- Вкладка **«Безопасность»** переработана: KPI по whitelist, числу банов и паузе после разбана, быстрый мастер-переключатель IP-ограничений, формы сохранения параметров антискана и разбана отдельных адресов.
- Страница **403 / IP заблокирован** приведена к единому виду с подсказками для пользователя.

### Telegram Mini App и авторизация

- Проверка подлинности **Telegram.WebApp.initData** вынесена в сервис `core/services/telegram_webapp_init_data.py` по [алгоритму Bot API](https://core.telegram.org/bots/webapps#validating-data-received-via-the-mini-app): сбор `data_check_string`, HMAC с ключом `WebAppData`, контроль `auth_date` с учётом `TELEGRAM_AUTH_MAX_AGE_SECONDS`.
- Обновлены шаблон и статика Mini App (`tg_mini_app.css`, `tg_mini_app.js`): вёрстка и поведение ближе к актуальным клиентам Telegram.

### Telegram-уведомления администраторам

- При настроенном боте авторизации возможна отправка уведомлений о **входе в панель** (успех и неудача) в Telegram.
- Во вкладке **«Пользователи»** добавлены настройки **Telegram-уведомлений** по пользователю: кулдаун между сообщениями, кнопка **«Тест уведомлений»**.

### Мобильный интерфейс и уведомления в UI

- Отдельные стили для **журнала аудита** на узких экранах (`audit_log_mobile.css`), доработаны мобильные стили **логов**, **монитора сервера**, **настроек** и **базового меню** (в т.ч. прозрачность фона оверлея меню).
- Общий слой **тостов/уведомлений** вынесен в `notifications.css` и подключён в `base.html`.

### Маршрутизация и списки IP

- На странице **«Маршрутизация»** уточнено отображение сводной информации по конфигурации AntiZapret и спискам (удобнее ориентироваться в сгенерированных маршрутах).
- Исправления генерации IP-списков для AntiZapret и обработки CIDR (в т.ч. пограничные случаи агрегации).

### Вкладка «Тесты»

- Расширен интерфейс: **список тестов** с количеством, **индикатор прогресса**, уточнённое описание области покрытия (ядро, CIDR, IP-защита, авторизация), блок с сырым выводом pytest.

### Прочее

- С главной страницы убран переключатель **«Режим экрана»** и связанный код.
- Тесты: добавлены сценарии для блокировки сканера, хранилища firewall, проверки `initData` Mini App и информации о версии панели; часть устаревших тестов удалена или заменена.
